...
/*******************************************************************************
* Function Name: HF_DAC_2_DacClk_Start
********************************************************************************
*
* Summary:
...
void HF_DAC_2_DacClk_Start(void) 
{
    ...
    HF_DAC_2_DacClk_CLKEN |= HF_DAC_2_DacClk_CLKEN_MASK;
	HF_DAC_2_DacClk_CLKSTBY |= HF_DAC_2_DacClk_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: HF_DAC_2_DacClk_Stop
********************************************************************************
*
* Summary:
...
void HF_DAC_2_DacClk_Stop(void) 
{
    ...
    HF_DAC_2_DacClk_CLKEN &= (uint8)(~HF_DAC_2_DacClk_CLKEN_MASK);
	HF_DAC_2_DacClk_CLKSTBY &= (uint8)(~HF_DAC_2_DacClk_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: HF_DAC_2_DacClk_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((HF_DAC_2_DacClk_CLKEN & HF_DAC_2_DacClk_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = HF_DAC_2_DacClk_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = HF_DAC_2_DacClk_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* HF_DAC_2_DacClk__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(HF_DAC_2_DacClk_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(HF_DAC_2_DacClk_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: HF_DAC_2_DacClk_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: HF_DAC_2_DacClk_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = HF_DAC_2_DacClk_GetSourceRegister();
    uint16 oldDivider = HF_DAC_2_DacClk_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = HF_DAC_2_DacClk_CLKEN & HF_DAC_2_DacClk_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(HF_DAC_2_DacClk_DIV_PTR, clkDivider);
                HF_DAC_2_DacClk_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                HF_DAC_2_DacClk_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(HF_DAC_2_DacClk_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((HF_DAC_2_DacClk_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: HF_DAC_2_DacClk_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(HF_DAC_2_DacClk_DIV_PTR);
}


/*******************************************************************************
* Function Name: HF_DAC_2_DacClk_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    HF_DAC_2_DacClk_MOD_SRC |= modeBitMask & (uint8)HF_DAC_2_DacClk_MODE_MASK;
}


/*******************************************************************************
* Function Name: HF_DAC_2_DacClk_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    HF_DAC_2_DacClk_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(HF_DAC_2_DacClk_MODE_MASK));
}


/*******************************************************************************
* Function Name: HF_DAC_2_DacClk_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return HF_DAC_2_DacClk_MOD_SRC & (uint8)(HF_DAC_2_DacClk_MODE_MASK);
}


/*******************************************************************************
* Function Name: HF_DAC_2_DacClk_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = HF_DAC_2_DacClk_GetDividerRegister();
    uint8 oldSrc = HF_DAC_2_DacClk_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        HF_DAC_2_DacClk_MOD_SRC |= CYCLK_SSS;
        HF_DAC_2_DacClk_MOD_SRC =
            (HF_DAC_2_DacClk_MOD_SRC & (uint8)(~HF_DAC_2_DacClk_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: HF_DAC_2_DacClk_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return HF_DAC_2_DacClk_MOD_SRC & HF_DAC_2_DacClk_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: HF_DAC_2_DacClk_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    HF_DAC_2_DacClk_PHASE = clkPhase & HF_DAC_2_DacClk_PHASE_MASK;
}


/*******************************************************************************
* Function Name: HF_DAC_2_DacClk_GetPhase
********************************************************************************
*
* Summary:
...
{
    return HF_DAC_2_DacClk_PHASE & HF_DAC_2_DacClk_PHASE_MASK;
}
