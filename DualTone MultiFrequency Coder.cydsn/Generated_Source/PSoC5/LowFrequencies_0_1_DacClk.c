/*******************************************************************************
* File Name: LowFrequencies_0_1_DacClk.c
* Version 2.10
*
*  Description:
...
/*******************************************************************************
* Function Name: LowFrequencies_0_1_DacClk_Start
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LowFrequencies_0_1_DacClk_Start(void) 
{
    /* Set the bit to enable the clock. */
    LowFrequencies_0_1_DacClk_CLKEN |= LowFrequencies_0_1_DacClk_CLKEN_MASK;
	LowFrequencies_0_1_DacClk_CLKSTBY |= LowFrequencies_0_1_DacClk_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: LowFrequencies_0_1_DacClk_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LowFrequencies_0_1_DacClk_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    LowFrequencies_0_1_DacClk_CLKEN &= (uint8)(~LowFrequencies_0_1_DacClk_CLKEN_MASK);
	LowFrequencies_0_1_DacClk_CLKSTBY &= (uint8)(~LowFrequencies_0_1_DacClk_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: LowFrequencies_0_1_DacClk_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LowFrequencies_0_1_DacClk_StopBlock(void) 
{
    if ((LowFrequencies_0_1_DacClk_CLKEN & LowFrequencies_0_1_DacClk_CLKEN_MASK) != 0u)
    {
...
#if defined(LowFrequencies_0_1_DacClk__CFG3)
        CLK_DIST_AMASK = LowFrequencies_0_1_DacClk_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = LowFrequencies_0_1_DacClk_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* LowFrequencies_0_1_DacClk__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(LowFrequencies_0_1_DacClk_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(LowFrequencies_0_1_DacClk_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: LowFrequencies_0_1_DacClk_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LowFrequencies_0_1_DacClk_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        LowFrequencies_0_1_DacClk_CLKSTBY &= (uint8)(~LowFrequencies_0_1_DacClk_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: LowFrequencies_0_1_DacClk_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LowFrequencies_0_1_DacClk_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = LowFrequencies_0_1_DacClk_GetSourceRegister();
    uint16 oldDivider = LowFrequencies_0_1_DacClk_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = LowFrequencies_0_1_DacClk_CLKEN & LowFrequencies_0_1_DacClk_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(LowFrequencies_0_1_DacClk_DIV_PTR, clkDivider);
                LowFrequencies_0_1_DacClk_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                LowFrequencies_0_1_DacClk_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(LowFrequencies_0_1_DacClk_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((LowFrequencies_0_1_DacClk_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(LowFrequencies_0_1_DacClk_DIV_PTR, clkDivider);
				LowFrequencies_0_1_DacClk_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: LowFrequencies_0_1_DacClk_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 LowFrequencies_0_1_DacClk_GetDividerRegister(void) 
{
    return CY_GET_REG16(LowFrequencies_0_1_DacClk_DIV_PTR);
}


/*******************************************************************************
* Function Name: LowFrequencies_0_1_DacClk_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LowFrequencies_0_1_DacClk_SetModeRegister(uint8 modeBitMask) 
{
    LowFrequencies_0_1_DacClk_MOD_SRC |= modeBitMask & (uint8)LowFrequencies_0_1_DacClk_MODE_MASK;
}


/*******************************************************************************
* Function Name: LowFrequencies_0_1_DacClk_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LowFrequencies_0_1_DacClk_ClearModeRegister(uint8 modeBitMask) 
{
    LowFrequencies_0_1_DacClk_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(LowFrequencies_0_1_DacClk_MODE_MASK));
}


/*******************************************************************************
* Function Name: LowFrequencies_0_1_DacClk_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 LowFrequencies_0_1_DacClk_GetModeRegister(void) 
{
    return LowFrequencies_0_1_DacClk_MOD_SRC & (uint8)(LowFrequencies_0_1_DacClk_MODE_MASK);
}


/*******************************************************************************
* Function Name: LowFrequencies_0_1_DacClk_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LowFrequencies_0_1_DacClk_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = LowFrequencies_0_1_DacClk_GetDividerRegister();
    uint8 oldSrc = LowFrequencies_0_1_DacClk_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        LowFrequencies_0_1_DacClk_MOD_SRC |= CYCLK_SSS;
        LowFrequencies_0_1_DacClk_MOD_SRC =
            (LowFrequencies_0_1_DacClk_MOD_SRC & (uint8)(~LowFrequencies_0_1_DacClk_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: LowFrequencies_0_1_DacClk_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 LowFrequencies_0_1_DacClk_GetSourceRegister(void) 
{
    return LowFrequencies_0_1_DacClk_MOD_SRC & LowFrequencies_0_1_DacClk_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: LowFrequencies_0_1_DacClk_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LowFrequencies_0_1_DacClk_SetPhaseRegister(uint8 clkPhase) 
{
    LowFrequencies_0_1_DacClk_PHASE = clkPhase & LowFrequencies_0_1_DacClk_PHASE_MASK;
}


/*******************************************************************************
* Function Name: LowFrequencies_0_1_DacClk_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 LowFrequencies_0_1_DacClk_GetPhaseRegister(void) 
{
    return LowFrequencies_0_1_DacClk_PHASE & LowFrequencies_0_1_DacClk_PHASE_MASK;
}
