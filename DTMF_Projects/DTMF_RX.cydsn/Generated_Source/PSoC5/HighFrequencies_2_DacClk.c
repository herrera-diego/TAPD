/*******************************************************************************
* File Name: HighFrequencies_2_DacClk.c
* Version 2.10
*
*  Description:
...
/*******************************************************************************
* Function Name: HighFrequencies_2_DacClk_Start
********************************************************************************
*
* Summary:
...
void HighFrequencies_2_DacClk_Start(void) 
{
    ...
    HighFrequencies_2_DacClk_CLKEN |= HighFrequencies_2_DacClk_CLKEN_MASK;
	HighFrequencies_2_DacClk_CLKSTBY |= HighFrequencies_2_DacClk_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: HighFrequencies_2_DacClk_Stop
********************************************************************************
*
* Summary:
...
void HighFrequencies_2_DacClk_Stop(void) 
{
    ...
    HighFrequencies_2_DacClk_CLKEN &= (uint8)(~HighFrequencies_2_DacClk_CLKEN_MASK);
	HighFrequencies_2_DacClk_CLKSTBY &= (uint8)(~HighFrequencies_2_DacClk_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: HighFrequencies_2_DacClk_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((HighFrequencies_2_DacClk_CLKEN & HighFrequencies_2_DacClk_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = HighFrequencies_2_DacClk_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = HighFrequencies_2_DacClk_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* HighFrequencies_2_DacClk__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(HighFrequencies_2_DacClk_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(HighFrequencies_2_DacClk_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: HighFrequencies_2_DacClk_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: HighFrequencies_2_DacClk_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = HighFrequencies_2_DacClk_GetSourceRegister();
    uint16 oldDivider = HighFrequencies_2_DacClk_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = HighFrequencies_2_DacClk_CLKEN & HighFrequencies_2_DacClk_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(HighFrequencies_2_DacClk_DIV_PTR, clkDivider);
                HighFrequencies_2_DacClk_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                HighFrequencies_2_DacClk_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(HighFrequencies_2_DacClk_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((HighFrequencies_2_DacClk_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(HighFrequencies_2_DacClk_DIV_PTR, clkDivider);
				HighFrequencies_2_DacClk_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: HighFrequencies_2_DacClk_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(HighFrequencies_2_DacClk_DIV_PTR);
}


/*******************************************************************************
* Function Name: HighFrequencies_2_DacClk_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    HighFrequencies_2_DacClk_MOD_SRC |= modeBitMask & (uint8)HighFrequencies_2_DacClk_MODE_MASK;
}


/*******************************************************************************
* Function Name: HighFrequencies_2_DacClk_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    HighFrequencies_2_DacClk_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(HighFrequencies_2_DacClk_MODE_MASK));
}


/*******************************************************************************
* Function Name: HighFrequencies_2_DacClk_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return HighFrequencies_2_DacClk_MOD_SRC & (uint8)(HighFrequencies_2_DacClk_MODE_MASK);
}


/*******************************************************************************
* Function Name: HighFrequencies_2_DacClk_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = HighFrequencies_2_DacClk_GetDividerRegister();
    uint8 oldSrc = HighFrequencies_2_DacClk_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        HighFrequencies_2_DacClk_MOD_SRC |= CYCLK_SSS;
        HighFrequencies_2_DacClk_MOD_SRC =
            (HighFrequencies_2_DacClk_MOD_SRC & (uint8)(~HighFrequencies_2_DacClk_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: HighFrequencies_2_DacClk_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return HighFrequencies_2_DacClk_MOD_SRC & HighFrequencies_2_DacClk_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: HighFrequencies_2_DacClk_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    HighFrequencies_2_DacClk_PHASE = clkPhase & HighFrequencies_2_DacClk_PHASE_MASK;
}


/*******************************************************************************
* Function Name: HighFrequencies_2_DacClk_GetPhase
********************************************************************************
*
* Summary:
...
{
    return HighFrequencies_2_DacClk_PHASE & HighFrequencies_2_DacClk_PHASE_MASK;
}
