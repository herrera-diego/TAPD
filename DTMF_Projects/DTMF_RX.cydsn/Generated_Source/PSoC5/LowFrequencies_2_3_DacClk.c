/*******************************************************************************
* File Name: LowFrequencies_2_3_DacClk.c
* Version 2.10
*
*  Description:
...
/*******************************************************************************
* Function Name: LowFrequencies_2_3_DacClk_Start
********************************************************************************
*
* Summary:
...
void LowFrequencies_2_3_DacClk_Start(void) 
{
    ...
    LowFrequencies_2_3_DacClk_CLKEN |= LowFrequencies_2_3_DacClk_CLKEN_MASK;
	LowFrequencies_2_3_DacClk_CLKSTBY |= LowFrequencies_2_3_DacClk_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: LowFrequencies_2_3_DacClk_Stop
********************************************************************************
*
* Summary:
...
void LowFrequencies_2_3_DacClk_Stop(void) 
{
    ...
    LowFrequencies_2_3_DacClk_CLKEN &= (uint8)(~LowFrequencies_2_3_DacClk_CLKEN_MASK);
	LowFrequencies_2_3_DacClk_CLKSTBY &= (uint8)(~LowFrequencies_2_3_DacClk_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: LowFrequencies_2_3_DacClk_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((LowFrequencies_2_3_DacClk_CLKEN & LowFrequencies_2_3_DacClk_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = LowFrequencies_2_3_DacClk_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = LowFrequencies_2_3_DacClk_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* LowFrequencies_2_3_DacClk__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(LowFrequencies_2_3_DacClk_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(LowFrequencies_2_3_DacClk_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: LowFrequencies_2_3_DacClk_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: LowFrequencies_2_3_DacClk_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = LowFrequencies_2_3_DacClk_GetSourceRegister();
    uint16 oldDivider = LowFrequencies_2_3_DacClk_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = LowFrequencies_2_3_DacClk_CLKEN & LowFrequencies_2_3_DacClk_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(LowFrequencies_2_3_DacClk_DIV_PTR, clkDivider);
                LowFrequencies_2_3_DacClk_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                LowFrequencies_2_3_DacClk_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(LowFrequencies_2_3_DacClk_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((LowFrequencies_2_3_DacClk_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(LowFrequencies_2_3_DacClk_DIV_PTR, clkDivider);
				LowFrequencies_2_3_DacClk_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: LowFrequencies_2_3_DacClk_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(LowFrequencies_2_3_DacClk_DIV_PTR);
}


/*******************************************************************************
* Function Name: LowFrequencies_2_3_DacClk_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    LowFrequencies_2_3_DacClk_MOD_SRC |= modeBitMask & (uint8)LowFrequencies_2_3_DacClk_MODE_MASK;
}


/*******************************************************************************
* Function Name: LowFrequencies_2_3_DacClk_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    LowFrequencies_2_3_DacClk_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(LowFrequencies_2_3_DacClk_MODE_MASK));
}


/*******************************************************************************
* Function Name: LowFrequencies_2_3_DacClk_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return LowFrequencies_2_3_DacClk_MOD_SRC & (uint8)(LowFrequencies_2_3_DacClk_MODE_MASK);
}


/*******************************************************************************
* Function Name: LowFrequencies_2_3_DacClk_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = LowFrequencies_2_3_DacClk_GetDividerRegister();
    uint8 oldSrc = LowFrequencies_2_3_DacClk_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        LowFrequencies_2_3_DacClk_MOD_SRC |= CYCLK_SSS;
        LowFrequencies_2_3_DacClk_MOD_SRC =
            (LowFrequencies_2_3_DacClk_MOD_SRC & (uint8)(~LowFrequencies_2_3_DacClk_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: LowFrequencies_2_3_DacClk_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return LowFrequencies_2_3_DacClk_MOD_SRC & LowFrequencies_2_3_DacClk_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: LowFrequencies_2_3_DacClk_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    LowFrequencies_2_3_DacClk_PHASE = clkPhase & LowFrequencies_2_3_DacClk_PHASE_MASK;
}


/*******************************************************************************
* Function Name: LowFrequencies_2_3_DacClk_GetPhase
********************************************************************************
*
* Summary:
...
{
    return LowFrequencies_2_3_DacClk_PHASE & LowFrequencies_2_3_DacClk_PHASE_MASK;
}
