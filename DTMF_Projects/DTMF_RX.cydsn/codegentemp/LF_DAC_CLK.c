...
/*******************************************************************************
* Function Name: LF_DAC_CLK_Start
********************************************************************************
*
* Summary:
...
void LF_DAC_CLK_Start(void) 
{
    ...
    LF_DAC_CLK_CLKEN |= LF_DAC_CLK_CLKEN_MASK;
	LF_DAC_CLK_CLKSTBY |= LF_DAC_CLK_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: LF_DAC_CLK_Stop
********************************************************************************
*
* Summary:
...
void LF_DAC_CLK_Stop(void) 
{
    ...
    LF_DAC_CLK_CLKEN &= (uint8)(~LF_DAC_CLK_CLKEN_MASK);
	LF_DAC_CLK_CLKSTBY &= (uint8)(~LF_DAC_CLK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: LF_DAC_CLK_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((LF_DAC_CLK_CLKEN & LF_DAC_CLK_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = LF_DAC_CLK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = LF_DAC_CLK_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* LF_DAC_CLK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(LF_DAC_CLK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(LF_DAC_CLK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: LF_DAC_CLK_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: LF_DAC_CLK_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = LF_DAC_CLK_GetSourceRegister();
    uint16 oldDivider = LF_DAC_CLK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = LF_DAC_CLK_CLKEN & LF_DAC_CLK_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((LF_DAC_CLK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: LF_DAC_CLK_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(LF_DAC_CLK_DIV_PTR);
}


/*******************************************************************************
* Function Name: LF_DAC_CLK_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    LF_DAC_CLK_MOD_SRC |= modeBitMask & (uint8)LF_DAC_CLK_MODE_MASK;
}


/*******************************************************************************
* Function Name: LF_DAC_CLK_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    LF_DAC_CLK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(LF_DAC_CLK_MODE_MASK));
}


/*******************************************************************************
* Function Name: LF_DAC_CLK_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return LF_DAC_CLK_MOD_SRC & (uint8)(LF_DAC_CLK_MODE_MASK);
}


/*******************************************************************************
* Function Name: LF_DAC_CLK_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = LF_DAC_CLK_GetDividerRegister();
    uint8 oldSrc = LF_DAC_CLK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        LF_DAC_CLK_MOD_SRC |= CYCLK_SSS;
        LF_DAC_CLK_MOD_SRC =
            (LF_DAC_CLK_MOD_SRC & (uint8)(~LF_DAC_CLK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: LF_DAC_CLK_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return LF_DAC_CLK_MOD_SRC & LF_DAC_CLK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: LF_DAC_CLK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    LF_DAC_CLK_PHASE = clkPhase & LF_DAC_CLK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: LF_DAC_CLK_GetPhase
********************************************************************************
*
* Summary:
...
{
    return LF_DAC_CLK_PHASE & LF_DAC_CLK_PHASE_MASK;
}
