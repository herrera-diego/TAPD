...
/*******************************************************************************
* Function Name: HF_DAC_CLK_Start
********************************************************************************
*
* Summary:
...
void HF_DAC_CLK_Start(void) 
{
    ...
    HF_DAC_CLK_CLKEN |= HF_DAC_CLK_CLKEN_MASK;
	HF_DAC_CLK_CLKSTBY |= HF_DAC_CLK_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: HF_DAC_CLK_Stop
********************************************************************************
*
* Summary:
...
void HF_DAC_CLK_Stop(void) 
{
    ...
    HF_DAC_CLK_CLKEN &= (uint8)(~HF_DAC_CLK_CLKEN_MASK);
	HF_DAC_CLK_CLKSTBY &= (uint8)(~HF_DAC_CLK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: HF_DAC_CLK_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((HF_DAC_CLK_CLKEN & HF_DAC_CLK_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = HF_DAC_CLK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = HF_DAC_CLK_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* HF_DAC_CLK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(HF_DAC_CLK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(HF_DAC_CLK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: HF_DAC_CLK_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: HF_DAC_CLK_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = HF_DAC_CLK_GetSourceRegister();
    uint16 oldDivider = HF_DAC_CLK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = HF_DAC_CLK_CLKEN & HF_DAC_CLK_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((HF_DAC_CLK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: HF_DAC_CLK_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(HF_DAC_CLK_DIV_PTR);
}


/*******************************************************************************
* Function Name: HF_DAC_CLK_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    HF_DAC_CLK_MOD_SRC |= modeBitMask & (uint8)HF_DAC_CLK_MODE_MASK;
}


/*******************************************************************************
* Function Name: HF_DAC_CLK_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    HF_DAC_CLK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(HF_DAC_CLK_MODE_MASK));
}


/*******************************************************************************
* Function Name: HF_DAC_CLK_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return HF_DAC_CLK_MOD_SRC & (uint8)(HF_DAC_CLK_MODE_MASK);
}


/*******************************************************************************
* Function Name: HF_DAC_CLK_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = HF_DAC_CLK_GetDividerRegister();
    uint8 oldSrc = HF_DAC_CLK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        HF_DAC_CLK_MOD_SRC |= CYCLK_SSS;
        HF_DAC_CLK_MOD_SRC =
            (HF_DAC_CLK_MOD_SRC & (uint8)(~HF_DAC_CLK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: HF_DAC_CLK_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return HF_DAC_CLK_MOD_SRC & HF_DAC_CLK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: HF_DAC_CLK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    HF_DAC_CLK_PHASE = clkPhase & HF_DAC_CLK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: HF_DAC_CLK_GetPhase
********************************************************************************
*
* Summary:
...
{
    return HF_DAC_CLK_PHASE & HF_DAC_CLK_PHASE_MASK;
}
